...
#elif defined(EXAMPLE_Rotate_270)
...
void lcd_lvgl_Init(void)
{
  ...
  lv_init();
  // Reduce buffer size to avoid memory allocation issues
  size_t buffer_size = EXAMPLE_LCD_H_RES * 60 * sizeof(lv_color_t); // Reduced from EXAMPLE_LVGL_BUF_HEIGHT
  lv_color_t *buf1 = heap_caps_malloc(buffer_size, MALLOC_CAP_DMA);
  assert(buf1);
  lv_color_t *buf2 = heap_caps_malloc(buffer_size, MALLOC_CAP_DMA);
  if (buf2 == NULL) {
    // If second buffer allocation fails, use single buffer mode
    ESP_LOGW("lcd_bsp", "Failed to allocate second buffer, using single buffer mode");
    lv_disp_draw_buf_init(&disp_buf, buf1, NULL, EXAMPLE_LCD_H_RES * 60);
  } else {
  ...
  // Demo code removed - UI initialization will be handled in main.cpp
}
...
bool get_lvgl_lock(int timeout_ms)
{
  return example_lvgl_lock(timeout_ms);
}

void release_lvgl_lock(void)
{
  example_lvgl_unlock();
}
